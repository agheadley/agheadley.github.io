class Stats {
  ...
  tValue=(dataList:number[])=> {
    ...
    if(sInfo.isValid===true && tInfo.isValid===true) {
      let tValue = Math.round(100 * sInfo.mean / (sInfo.s / Math.sqrt(sInfo.n))) / 100;
      let conf=tInfo.t*sInfo.s/Math.sqrt(sInfo.n);
      return {isValid:true,confUp:(sInfo.mean-conf),confDown:(sInfo.mean+conf),s:sInfo.s,mean:sInfo.mean,n:sInfo.n,tTest:tInfo.t,tValue:tValue};
    }
    else return {isValid:false,confUp:null,confDown:null,s:null,mean:null,n:null,tTest:null,tValue:null};
  }
  
  // calculates s,mean and n.
  sdSample=(dataList:number[])=> {
    ...
    if (data.length > 1) {
      ...
      let s = Math.sqrt((squares - data.length * mean * mean) / (data.length - 1));
